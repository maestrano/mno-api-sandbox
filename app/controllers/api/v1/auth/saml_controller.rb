class Api::V1::Auth::SamlController < SamlIdp::IdpController
  ...
  def index
    begin
      if current_app
        logger.info("INSPECT: app => #{current_app.to_json}")

        # if a user_uid is explicitly selected then proceed to SSO response
        # Otherwise ask user to select a system user to login with
        if params[:user_uid] && current_user
          logger.info("INSPECT: user UID => #{current_user.uid}")
  ...
  protected
    def current_app
      @app ||= @saml_issuer_id.blank? ? nil : App.find_by_uid(@saml_issuer_id)
    end
